...
async function seed() {
    try {
        for (const u of demoUsers) {
            ...
            // Upsert: insert if missing, otherwise fix the password/is_active
            // on the existing row. Requires a UNIQUE constraint on username
            // (add one if you don't have it: ALTER TABLE users ADD CONSTRAINT
